...
const searchUrl: string = process.env.NEXT_PUBLIC_USER_DETAILS_URL || "http://localhost:80/v1/search/all";
...
<div ref={searchRef} className="relative w-1/3">
  ...
  {showResults && (
    <div className="absolute min-w-full bg-gray-800 text-white shadow-lg rounded-lg mt-2 z-50 p-2 border border-gray-600">
      {/* User Results */}
      {(results?.userIds && results?.userIds?.length > 0) && (
        <div>
          <h3 className="text-gray-300 font-semibold px-2">Users</h3>
          {results?.userIds?.map((user) => (
            <Link href={`/profile/${user}`} key={user} className="block px-4 py-2 hover:bg-gray-700 rounded-md">
              {user} (@{user})
            </Link>
          ))}
        </div>
      ...
      {/* Post Results */}
      {(results?.postIds && results.postIds.length > 0) && (
        <div className="mt-2">
          <h3 className="text-gray-300 font-semibold px-2">Posts</h3>
          {results?.postIds?.map((post) => (
            <Link href={`/post/${post}`} key={post} className="block px-4 py-2 hover:bg-gray-700 rounded-md">
              {post}
            </Link>
          ))}
        </div>
      ...
      {/* "See More" Button */}
      {((results?.userIds && results.userIds.length  > 0) || (results?.postIds && results.postIds.length > 0)) && (
        <div className="mt-2 text-center">
          <Link href={`/search?q=${query}`} className="block py-2 text-blue-400 hover:underline">
            See More Results
          </Link>
        </div>
      )}
