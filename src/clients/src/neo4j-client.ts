...
export async function connectNeo4jDriver(): Promise<Driver> {
  logger.info(
    `Connecting to to Neo4j driver on ${neo4j_host_port}, username: ${neo4j_username}, password: ${neo4j_password}`,
  );
  ...
  logger.info(
    `Connected to Neo4j driver on ${info.address}, agent ${info.agent}`,
  );
