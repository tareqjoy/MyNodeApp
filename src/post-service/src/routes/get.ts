...
import { TooLargeRequest, GetPostReq, InternalServerError } from '@tareqjoy/models';
    ...
    router.post('/', async (req, res, next) => {
        ...
        try {
            ...
            if (axios.isAxiosError(error)) {
                logger.error(`Error while /get: url: ${error.config?.url}, status: ${error.response?.status}, message: ${error.message}`);
            } else {
