import { Attachment, LinkedEntity, Post } from "@tareqjoy/clients";
import { NewPostKafkaMsg } from "@tareqjoy/models";
import { getFileLogger, withRetry } from "@tareqjoy/utils";
import { Producer } from "kafkajs";
import mongoose, { Mongoose } from "mongoose";


const logger = getFileLogger(__filename);

export async function writePost(
  loggedInUserId: string,
  body: string,
  postTime: number,
  attachmentIds: string[] | undefined,
  postType: string,
  newPostKafkaProducer: Producer,
  fanoutTopic: string
): Promise<String> {
  const session = await mongoose.startSession();
  session.startTransaction();

  try {
    const post = new Post({
      _id: new mongoose.Types.ObjectId(),
      userId: loggedInUserId,
      body,
      time: postTime,
      attachments: attachmentIds?.map((id) => new mongoose.Types.ObjectId(id)),
      postType,
    });

    const dbResult = await withRetry(() => post.save({ session }));

    if (attachmentIds && attachmentIds.length > 0) {
      await withRetry(() =>
        Attachment.updateMany(
          { _id: { $in: attachmentIds } },
          {
            $set: {
              linkedTo: LinkedEntity.POST,
              linkedId: dbResult._id,
            },
          },
          { session }
        )
      );
    }

    await session.commitTransaction();
    session.endSession();

    logger.debug(`saved to mongodb with postId: ${dbResult.id}`);

    const kafkaMsg = new NewPostKafkaMsg(
      dbResult.id,
      dbResult.userId!.toString(),
      dbResult.time,
      dbResult.postType,
      dbResult.attachments!.map((id) => id.toString())
    );

    logger.debug(`publishing Kafka: topic=${fanoutTopic}`);
    await withRetry(() =>
      newPostKafkaProducer.send({
        topic: fanoutTopic,
        messages: [
          {
            key: dbResult.id,
            value: JSON.stringify(kafkaMsg),
          },
        ],
      })
    );

    return dbResult.id.toString();
  } catch (err) {
    await session.abortTransaction();
    session.endSession();
    throw err;
  }
}
