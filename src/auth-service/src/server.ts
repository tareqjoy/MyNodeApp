...
async function main() {
  ...
  app.use(
    getApiPath(api_path_auth_root, "signin"),
    createSignInRouter(redisClient),
  );
  app.use(getApiPath(api_path_auth_root, "verify"), createVerifyRouter());
  app.use(
    getApiPath(api_path_auth_root, "refresh"),
    createRefreshRouter(redisClient),
  );
  app.use(
    getApiPath(api_path_auth_root, "signout"),
    createSignOutRouter(redisClient),
  );
  app.use(
    getApiPath(api_path_auth_root, "authorize"),
    createAuthorizeClientRouter(redisClient),
  );
