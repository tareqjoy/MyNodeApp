...
const authorize_code_exp_sec = Number(
  process.env.AUTH_CODE_EXPIRES_SEC || "120",
); //2min
      ...
      if (authorizeClientObj.response_type === "code") {
        ...
        if (!authHeader || typeof authHeader !== "string") {
          res
            ...
            .json(
              new InvalidRequest(
                `Header ${ATTR_HEADER_AUTHORIZATION} is required`,
              ),
            );
          return;
        }
        ...
        if (authorizeClientObj.client_id) {
          ...
          const base64str =
            req.headers[ATTR_HEADER_AUTHORIZATION].split(" ")[1];
          const clientIdSecret = Buffer.from(base64str, "base64").toString(
            "utf-8",
          );
        ...
        if (!clientId) {
          res
            ...
            .json(
              new InvalidRequest(
                "Client id is required either in header authorization (Basic <base64>) or in body client_id (plain text)",
              ),
            );
          return;
        }
          ...
          res
            ...
            .json(
              new UnauthorizedRequest(
                "request didn't match with the access code expectation",
              ),
            );
          return;
        }
        ...
        res
          ...
          .json(
            new InvalidRequest(
              "response_type=code or grant_type=authorization_code is required",
            ),
          );
        return;
      }
